...
!!! danger "Lưu Ý:"
    - Đừng xoá thư mục này một cách trực tiếp. Phải `umount` trước rồi hãn xoá thư mục. Thế nên cần phải để thư mục này ở dưới quyền `root`

    - Để ngắt kết nối thì trường hợp này dùng lệnh:

    ```bash
    sudo mount -t cifs //$IP/$NAME
    ```